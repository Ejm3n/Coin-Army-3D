...
namespace ntw.CurvedTextMeshPro
{
    ...
    /// </summary>
    [ExecuteInEditMode]
    public class TextProCurveBack : TextProOnACurve
    {
        /// <summary>
        ...
        protected override Matrix4x4 ComputeTransformationMatrix(Vector3 charMidBaselinePos, float zeroToOnePos, TMP_TextInfo textInfo, int charIdx)      
        {
            ...
            Vector2 newMideBaselinePos = new Vector2(x0 * radiusForThisLine, -z0 * radiusForThisLine); //actual new position of the character
            ...
            return Matrix4x4.TRS(new Vector3(newMideBaselinePos.x, 0, -newMideBaselinePos.y), Quaternion.AngleAxis(-Mathf.Atan2(z0, x0) * Mathf.Rad2Deg - 90, Vector3.up), Vector3.one);
        }
    }
}
