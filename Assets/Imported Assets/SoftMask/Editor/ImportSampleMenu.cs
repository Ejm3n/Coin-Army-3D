...
namespace Coffee.UISoftMask
{
    public static class ImportSampleMenu
    {
        ...
        private static void ImportSample(string jsonGuid, string sampleName)
        {
            ...
            var src = StringBuffer.Format("{0}/Samples~/{1}", Path.GetDirectoryName(jsonPath), sampleName);
            var srcAlt = StringBuffer.Format("{0}/Samples/{1}", Path.GetDirectoryName(jsonPath), sampleName);
            var dst = StringBuffer.Format("Assets/Samples/{0}/{1}/{2}", displayName, version, sampleName);
        ...
        {
            var sampleRoot = StringBuffer.Format("Assets/Samples/{0}", displayName);
